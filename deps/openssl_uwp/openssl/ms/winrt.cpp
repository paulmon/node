...
unsigned entropyRT(BYTE *buffer, unsigned len)
	...
	ComPtr<ICryptographicBufferStatics> cryptoBufferFactory;
	IBuffer* buf;
	UINT32 arrayLen;
	BYTE* out;

	hr = Windows::Foundation::GetActivationFactory(HStringReference(
		RuntimeClass_Windows_Security_Cryptography_CryptographicBuffer).Get(), &cryptoBufferFactory);
	if (FAILED(hr)) {
		return -1;
	}

	hr = Windows::Foundation::GetActivationFactory(HStringReference(
		RuntimeClass_Windows_Storage_Streams_Buffer).Get(), &bufferFactory);
	if (FAILED(hr)) {
		return -1;
	}

	hr = bufferFactory->Create(len, &buf);
	if (FAILED(hr)) {
		return -1;
	}

	hr = cryptoBufferFactory->GenerateRandom(len, &buf);
	if (FAILED(hr)) {
		return -1;
	}

	hr = cryptoBufferFactory->CopyToByteArray(buf, &arrayLen, &out);
	if (FAILED(hr)) {
		return -1;
	}

	memcpy_s(buffer, len, out, arrayLen);
	return arrayLen;
	}
#endif
